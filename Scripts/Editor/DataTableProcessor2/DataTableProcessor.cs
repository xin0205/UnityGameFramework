...
namespace UnityGameFramework.Editor.DataTableTools2
{
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(string dataTableFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            ...
            {
                string[] rawValue = lines[i].Split(Const.DataSplitSeparators);
                for (int j = 0; j < rawValue.Length; j++)
                {
                    rawValue[j] = rawValue[j].Trim(Const.DataTrimSeparators);
                }
            ...
            for (int i = 0; i < rawColumnCount; i++)
            {
                if (i == IdColumn)
                {
                    m_DataProcessor[i] = DataProcessorUtility.GetDataProcessor(Const.IDSign);
                }
                else
                {
        ...
        public bool IsIdColumn(int rawColumn)
        {
            ...
            return m_DataProcessor[rawColumn].ContentType == ContentType.ID;
        }
        ...
        public bool IsCommentRow(int rawRow)
        {
            ...
            return GetValue(rawRow, 0).StartsWith(Const.CommentSign);
        }
        ...
        public bool IsCommentColumn(int rawColumn)
        {
            ...
            return string.IsNullOrEmpty(GetName(rawColumn)) || m_DataProcessor[rawColumn].ContentType == ContentType.Comment;
        }

        public DataType GetValueType(int rawColumn)
        {
            if (rawColumn < 0 || rawColumn >= RawColumnCount)
            {
                throw new GameFrameworkException(Utility.Text.Format("Raw column '{0}' is out of range.", rawColumn.ToString()));
            ...
            return m_DataProcessor[rawColumn].DataType;
        }
        ...
        public bool IsSystem(int rawColumn)
        {
            ...
            return m_DataProcessor[rawColumn].DataType == DataType.System;
        }
        ...
        //        throw new GameFrameworkException(Utility.Text.Format("Raw column '{0}' is out of range.", rawColumn.ToString()));
        //    }
        //    string typeName = m_DataProcessor[rawColumn].LanguageKeyword.Substring(0, 1).ToUpper() + m_DataProcessor[rawColumn].LanguageKeyword.Substring(1).ToLower();
                    ...
                    using (BinaryWriter stream = new BinaryWriter(fileStream, encoding))
                    {
                        for (int i = ContentStartRow; i < RawRowCount; i++)
                        {
                            ...
                            for (int j = 0; j < RawColumnCount; j++)
                            {
                                ...
                                {
                                    if (m_DataProcessor[j].ContentType == ContentType.ID || string.IsNullOrEmpty(GetDefaultValue(j)))
                                    {
